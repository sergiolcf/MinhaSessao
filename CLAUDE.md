...
- Entidade `Models/Entities/Sessao.cs` (`PacienteId`, `ProfissionalId`, `DataHora`, `Status` — enum `Models/Entities/StatusSessao.cs`: `Agendada`/`Realizada`/`Cancelada`). Ainda não existe tela de agendamento pelo Profissional — a tabela `Sessoes` é só consumida (leitura) pelo Paciente por enquanto; a criação/gestão de sessões fica para um card futuro do lado do Profissional.
